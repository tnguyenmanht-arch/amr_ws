...
/* ARR của TIM3 = 4499 (PWM ~20kHz, xem tim.c: htim3.Init.Period = 4499) */
#define PWM_ARR         4499u

/* TIM8 chỉ đếm 16-bit (encoder phải) -> cần cộng dồn tràn số trong phần mềm.
 * TIM2 (encoder trái) đếm 32-bit nên đọc thẳng CNT là đủ, không cần cộng dồn. */
static int32_t  right_enc_accum   = 0;
static uint16_t right_enc_last_cnt = 0;

HAL_StatusTypeDef DRV_Motor_Init(void)
{
    /* Encoder trái (TIM2, 32-bit) + encoder phải (TIM8, 16-bit) */
...
{
    /* Bánh trái/phải lắp đối xứng nhau trên khung xe -> cùng 1 lệnh "tiến"
     * khiến 2 trục quay ngược chiều nhau khi nhìn từ ngoài (đã verify thực
     * nghiệm 2026-07-xx: enc_l dương, enc_r âm cùng lúc khi tiến). Đảo dấu
     * bên phải ở đây để cả 2 bánh cùng đẩy xe đi thẳng đúng chiều. */
...
HAL_StatusTypeDef DRV_Motor_GetEncoder(int32_t *left, int32_t *right)
{
    ...
    *left = (int32_t)__HAL_TIM_GET_COUNTER(&htim2);
    ...
    /* Phải: TIM8 16-bit, cộng dồn delta để không mất số khi tràn */
    uint16_t cur = (uint16_t)__HAL_TIM_GET_COUNTER(&htim8);

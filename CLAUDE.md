...
### ✅ Giai đoạn 2 — STM32 Firmware: HOÀN THÀNH
- [x] Tạo project STM32CubeIDE, config I2C1 + USART1 + USART2
- [x] Driver I2C → Hiwonder Motor Driver (FIXED_SPEED, ENCODER_TOTAL) — `Core/Src/motor_driver.c`
- [x] Driver USART1 → HTS-20H qua TTL Bus Board (BusLinker, LEN=7) — `Core/Src/servo_buslinker.c`
- [x] Parser USART2: nhận lệnh từ Jetson, gửi encoder về — `Core/Src/jetson_comm.c`
- [ ] Test encoder odometry (chưa verify thực tế)
...
- [ ] `serial_driver_node` (amr_hardware) giao tiếp thực với STM32 qua UART2
- [ ] Verify odometry `/odom` + TF `odom → base_link`
- [ ] Driver LiDAR A1M8 hoạt động (`/scan`)
- [ ] Driver Camera IMX-219 hoạt động (`/camera/image_raw`)

**Việc tiếp theo:** Viết amr_hardware node trên Jetson (UART2 → STM32, parse cmd_vel → gửi lệnh, nhận encoder → publish /odom)

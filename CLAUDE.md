...
### 🔧 Giai đoạn 3 — ROS2 Hardware Nodes: ĐANG TRIỂN KHAI (đã phát hiện lỗi, chưa sửa)
...
- [ ] ⚠️ **`SerialDriver` (`serial_driver.hpp`/`stm32_comm.cpp`) ĐANG DÙNG PROTOCOL NHỊ PHÂN CŨ** (`0xAA 0x55`, `CMD_SET_VEL/CMD_ODOM`, CRC XOR) — **không tương thích** với firmware STM32 hiện tại (đã đổi sang ASCII `$VEL`/`$ODO` từ đầu phiên làm việc trước). Node compile/chạy được, port mở bình thường, nhưng **không trao đổi được dữ liệu gì** (lỗi ngầm, không báo lỗi rõ ràng) — `/odom` sẽ không bao giờ publish nếu chạy nguyên trạng.
- [ ] **Việc cần làm tiếp theo**: viết lại CHỈ 2 file `serial_driver.hpp` + `stm32_comm.cpp` sang ASCII line-based (giữ nguyên phần khung ROS2 `serial_driver_node.cpp`):
  + `sendCmdVel`: build chuỗi `"$VEL,%.2f,%.2f\n"` rồi `write()`
  + `readOdom`: gom byte tới `'\n'`, parse `"$ODO,%d,%d,%f"` — bỏ state machine header/CRC nhị phân
  + `OdomData.steering_pos` đang là `uint16_t` (vị trí servo cũ) → đổi thành `float steer_deg` (STM32 gửi góc có dấu)
  + Cân nhắc publish thêm `/steering_angle` (topic đã khai trong mục 3 nhưng chưa implement)
- [ ] Verify odometry `/odom` + TF `odom → base_link` (chặn bởi việc sửa `SerialDriver` ở trên)
- [x] Driver LiDAR A1M8 hoạt động (`/scan`)
- [ ] Driver Camera IMX-219 hoạt động (`/camera/image_raw`)

**Việc tiếp theo (ưu tiên):** Trên Jetson — sửa `SerialDriver` (2 file trên) sang ASCII, `colcon build`, chạy `serial_driver_node`, verify `/odom` publish + TF đúng, sau đó test Camera IMX-219. (Đã thảo luận giải pháp, CHƯA code — lưu ý cho phiên làm việc kế tiếp trên Jetson.)
